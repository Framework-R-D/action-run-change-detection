# Release Procedure for action-run-change-detection
...
gh release create "${NEXT_VERSION}" \
  ...
  --generate-notes \
  --repo Framework-R-D/action-run-change-detection
...
gh release view "${NEXT_VERSION}" --repo Framework-R-D/action-run-change-detection
...
NEW_SHA=$(gh api repos/Framework-R-D/action-run-change-detection/git/ref/tags/${NEXT_VERSION} \
  --jq .object.sha)
# If the tag is annotated, dereference:
TYPE=$(gh api repos/Framework-R-D/action-run-change-detection/git/ref/tags/${NEXT_VERSION} \
  --jq .object.type)
if [ "$TYPE" = "tag" ]; then
  NEW_SHA=$(gh api repos/Framework-R-D/action-run-change-detection/git/tags/${NEW_SHA} \
    --jq .object.sha)
fi

# Update README.md
OLD_PATTERN="Framework-R-D/action-run-change-detection@[0-9a-f]\{40\} # v[0-9]*"
NEW_STRING="Framework-R-D/action-run-change-detection@${NEW_SHA} # ${NEXT_VERSION}"
